...
def get_prompter_session(task_id: str) -> tuple[str | None, bool]:
    ...
    Returns:
        (session_id, should_resume): session_id if exists, and True if should resume
    """
    session_id = load_session("PROMPTER", task_id)
    if session_id:
        return session_id, True
    return None, False


def save_prompter_session(task_id: str, session_id: str) -> None:
    """Save prompter session for a task."""
    save_session("PROMPTER", task_id, session_id, metadata=f"Prompter for {task_id}")
