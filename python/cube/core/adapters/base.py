...
async def read_stream_with_buffer(stdout: asyncio.StreamReader, buffer_size: int = 8192) -> AsyncGenerator[str, None]:
...
async def run_subprocess_streaming(
    ...
    # Keep last N lines for error context
    recent_lines: deque[str] = deque(maxlen=10)

    health_monitor = asyncio.create_task(
        monitor_process_health(
            process, tool_name, check_interval=5.0, last_output_tracker=last_output_time, output_timeout=output_timeout
        )
    )
    ...
    if exit_code != 0:
        # Include recent output for error context
        context = ""
        if recent_lines:
            # Look for error-like lines
            error_lines = [line for line in recent_lines if "error" in line.lower() or "failed" in line.lower()]
            if error_lines:
                context = f" - {error_lines[-1][:200]}"
            else:
                context = f" - last output: {list(recent_lines)[-1][:100]}"
        raise RuntimeError(f"{tool_name} exited with code {exit_code}{context}")
