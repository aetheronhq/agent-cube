...
def load_config() -> CubeConfig:
    ...
    for idx, (key, w) in enumerate(data.get("writers", {}).items()):
        ...
        writers[key] = WriterConfig(key=key, name=w["name"], model=w["model"], label=w["label"], letter=letter, color=w["color"])
    ...
    prompter_model = prompter.get("model", "sonnet-4.6-thinking")
...
def get_writer_by_key_or_metadata(key: str, task_id: str) -> WriterConfig:
    """Get writer config by key, falling back to metadata for orphaned writers."""
    try:
        return resolve_writer_alias(key)
    except KeyError:
        from .writer_metadata import load_writer_metadata

        metadata = load_writer_metadata(key.lower(), task_id)
        if metadata:
            return WriterConfig(
                key=metadata.key,
                name=metadata.name,
