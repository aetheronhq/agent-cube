...
async def send_feedback_async(writer: str, task_id: str, feedback_file: Path, session_id: str, worktree: Path) -> None:
    ...
    stream = run_agent(worktree, wconfig.model, feedback_message, session_id=session_id, resume=True)
...
def feedback_command(writer: str, task_id: str, feedback_file: str) -> None:
    ...
    try:
        asyncio.run(send_feedback_async(writer_name, task_id, feedback_path, session_id, worktree))
